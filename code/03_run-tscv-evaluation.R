# -----------------------------------------------------------------------------------------------------------
# Run time-series cross-validation (TSCV) model evaluation
# -----------------------------------------------------------------------------------------------------------
...
cat("TSCV evaluation for: ", mod_name)

sessionInfo()

source(here("code", "utils_fn.R"))
source(here("code", "evaluate-tscv_fn.R"))
...
output_path <- here("output", "tscv", start_date, mod_name)
...
tscv_out <- evaluate_tscv(data_input = df_model,
                          nb_graph = nb_graph,
                          forms = forms,
                          mod_name = mod_name,
...
adm2_preds <- get_adm2_preds(list(tscv_out), df_model, mod_name)
cat("Admin 2 predictions generated.")
print(head(adm2_preds))
adm1_preds <- get_adm1_preds(list(tscv_out), df_model, mod_name)
cat("Admin 1 predictions generated.")
print(head(adm1_preds))
rm(tscv_out)

preds_out <- list(adm2_preds = adm2_preds, adm1_preds = adm1_preds)
qsave(preds_out, here(output_path, paste0("tscv-predictions",  "_",mod_name, ".qs")))
...
ggsave(here(output_path, paste0("tscv-adm2-predictions-plot",  "_",mod_name,".jpeg")), adm2_pred_plot, width = 36, height = 24)
ggsave(here(output_path, paste0("tscv-adm1-predictions-plot",  "_",mod_name,  ".jpeg")), adm1_pred_plot, width = 36, height = 24)
